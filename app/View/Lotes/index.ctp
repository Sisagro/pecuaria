...
<table cellpadding="0" cellspacing="0">
    ...
    <?php foreach ($lotes as $item): ?>
        <tr>
            <td><?php echo h($item['Lote']['id']); ?>&nbsp;</td>
            <td><?php echo h($item['Lote']['descricao']); ?>&nbsp;</td>
            <td><?php if ($item['Lote']['ativo'] == 'S') { echo h('SIM'); } else { echo h('NÃO'); } ?>&nbsp;</td>
            <td>
                <div id="botoes">
                    <?php
                    echo $this->Html->link($this->Html->image("botoes/view.png", array("alt" => "Visualizar", "title" => "Visualizar")), array('action' => 'view', $item['Lote']['id']), array('escape' => false));
                    echo $this->Html->link($this->Html->image("botoes/editar.gif", array("alt" => "Editar", "title" => "Editar")), array('action' => 'edit', $item['Lote']['id']), array('escape' => false));
                    echo $this->Form->postLink($this->Html->image('botoes/excluir.gif', array('alt' => 'Exluir', 'title' => 'Exluir')),
                                               array('action' => 'delete', $item['Lote']['id']), array('escape' => false),
