...
<table cellpadding="0" cellspacing="0">
    ...
    <?php foreach ($medicamentos as $item): ?>
        <tr>
            <td><?php echo h($item['Medicamento']['id']); ?>&nbsp;</td>
            <td><?php echo h($item['Medicamento']['descricao']); ?>&nbsp;</td>
            <td><?php echo h($item['Medicamento']['palavrachave']); ?>&nbsp;</td>
            <td><?php echo h($item['Grpeventosanitario']['descricao']); ?>&nbsp;</td>
            <td>
                <div id="botoes">
                    <?php
                    echo $this->Html->link($this->Html->image("botoes/view.png", array("alt" => "Visualizar", "title" => "Visualizar")), array('action' => 'view', $item['Medicamento']['id']), array('escape' => false));
                    echo $this->Html->link($this->Html->image("botoes/editar.gif", array("alt" => "Editar", "title" => "Editar")), array('action' => 'edit', $item['Medicamento']['id']), array('escape' => false));
                    echo $this->Form->postLink($this->Html->image('botoes/excluir.gif', array('alt' => 'Exluir', 'title' => 'Exluir')),
                                               array('action' => 'delete', $item['Medicamento']['id']), array('escape' => false),
